...
| `tools/assets-manifest.json` | Records the original extraction mapping from readable paths to Scratch costume/sound names, source hashes and owning sprites. It is provenance metadata, not a runtime dependency or extraction script. It predates the final renaming and removal of assets, so some paths are historical. |
